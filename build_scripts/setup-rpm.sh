...
# Purpose:
#
#     Prepare a fresh RPM-family host to build and work on unfathomably-be.
#
# Responsibilities:
#
#     * install operating-system packages through dnf, yum, or zypper
...
#     * deployment secrets
#
# RPM distributions do not agree on multimedia package availability. Fedora
# usually has enough packages in the default repositories for development,
# while RHEL-family systems may need EPEL, CRB, or RPM Fusion for packages such
# as ffmpeg, vips-devel, and perl-Image-ExifTool.
...
usage() {
    cat <<'EOF'
Usage: build_scripts/setup-rpm.sh [options]

Options:
    --no-os-packages   Skip RPM package installation.
...
require_rpm_family() {
    ...
    case "${ID:-}:${ID_LIKE:-}" in
        fedora:*|rhel:*|centos:*|rocky:*|almalinux:*|opensuse*:*|sles:*|*:fedora*|*:rhel*|*:suse*)
            return 0
            ;;
        *)
            echo "This bootstrap supports RPM-family distributions only. Detected ID=${ID:-unknown}." >&2
            exit 1
            ;;
    esac
}
...
rpm_install_required() {
    if command -v dnf5 >/dev/null 2>&1; then
        ...
        echo "Could not find dnf, yum, or zypper on this RPM-family host." >&2
        exit 1
    fi
}

rpm_install_optional() {
    local package

    for package in "$@"; do
        if ! rpm_install_required "$package"; then
            echo "Optional package was not available: $package" >&2
        fi
    done
}
...
main() {
    ...
    echo "unfathomably-be RPM-family setup complete."
}

main "$@"

# end of build_scripts/setup-rpm.sh
