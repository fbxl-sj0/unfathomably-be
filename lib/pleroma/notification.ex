...
defmodule Pleroma.Notification do
  ...
  # Single notification creation still goes through a transaction so the marker
  # update stays atomic with insertion.
  ...
  def get_potential_receiver_ap_ids(%{data: %{"type" => "Join", "object" => object_id}}) do
    case User.get_cached_by_ap_id(object_id) do
      ...
      _ ->
        join_event_receiver_ap_ids(object_id)
    end
  end
  ...
  def get_potential_receiver_ap_ids(activity) do
    ...
    |> Utils.maybe_notify_replied_to_author(activity)
    |> Utils.maybe_notify_followers(activity)
    |> Enum.uniq()
  end

  defp join_event_receiver_ap_ids(object_id) do
