...
defmodule Pleroma.Web.ApiSpec.CastAndValidate do
  ...
  def call(conn, %{operation_id: operation_id, render_error: render_error} = opts) do
    ...
    case cast_and_validate(spec, operation, conn, content_type, strict?(), cast_opts) do
      ...
      {:error, reason} ->
        Logger.error(
          "Strict ApiSpec request denied for #{conn.request_path} in #{operation_id}: #{inspect(reason)}"
        )
          ...
          Enum.reduce(errors, conn.query_params, fn
            ...
            %{reason: :invalid_enum, name: nil, path: path, value: value}, params ->
              ...
              case path do
                ...
                path ->
                  update_in(params, path, fn
                    values when is_list(values) -> List.delete(values, value)
