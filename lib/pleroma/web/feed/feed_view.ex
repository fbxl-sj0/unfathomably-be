...
defmodule Pleroma.Web.Feed.FeedView do
  ...
  def feed_logo do
    case Pleroma.Config.get([:feed, :logo]) do
      nil ->
        "#{Pleroma.Web.Endpoint.url()}/images/unfathomably-logo.svg"
  ...
  defp rss_attachment_preview(attachment) do
    ...
    if is_binary(href) and href != "" do
      ...
      escaped_name = escape(attachment["name"] || "Attachment")
  ...
  def activity_link(activity, data) do
    if activity.local do
      ...
      data["external_url"] || data["id"] || data["url"]
    end
  end
  ...
  def rss_enclosure_attachment(attachments) do
    Enum.find(attachments || [], &(is_binary(attachment_href(&1)) and attachment_href(&1) != ""))
  end
