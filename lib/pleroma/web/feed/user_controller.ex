...
defmodule Pleroma.Web.Feed.UserController do
  ...
  def feed_redirect(%{assigns: %{format: format}} = conn, params)
      when format not in ["html", "atom", "rss", "json", "activity+json"] do
    if remote_profile_path?(conn) do
      Pleroma.Web.Fallback.RedirectController.redirector(conn, nil)
    else
      conn
      |> Map.put(:assigns, Map.delete(conn.assigns, :format))
      |> feed_redirect(params)
    end
  end
  ...
  defp remote_profile_path?(%{path_info: ["@" <> nickname]}) do
