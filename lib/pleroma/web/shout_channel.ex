...
defmodule Pleroma.Web.ShoutChannel do
  ...
  def handle_in("new_msg", %{"text" => text}, %{assigns: %{user_name: user_name}} = socket) do
    ...
    with true <- valid_message?(text),
         %User{} = author <- User.get_cached_by_nickname(user_name) do
  ...
  defp valid_message?(text) do
    String.length(text) in 1..Pleroma.Config.get([:shout, :limit], 5_000)
  end
end
