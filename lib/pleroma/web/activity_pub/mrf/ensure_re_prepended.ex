...
defmodule Pleroma.Web.ActivityPub.MRF.EnsureRePrepended do
  ...
  defp filter_object_by_summary(object) do
    object["inReplyTo"]
    |> Object.normalize(fetch: false)
    |> filter_by_summary(object)
  end

  defp filter_history(%{"formerRepresentations" => %{"orderedItems" => items}} = object)
       when is_list(items) do
    put_in(
      object,
      ["formerRepresentations", "orderedItems"],
      Enum.map(items, &filter_object_by_summary/1)
    )
  end

  defp filter_history(object), do: object

  def filter(%{"type" => type, "object" => object} = activity)
      when type in ["Create", "Update"] and is_map(object) do
    child =
      object
      |> filter_object_by_summary()
      |> filter_history()
