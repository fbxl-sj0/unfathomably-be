defmodule Pleroma.Web.ActivityPub.MRF.RemoteReportPolicy do
  ...
  defp maybe_reject_third_party(%{"object" => objects} = object) do
    with true <- Config.get([:mrf_remote_report, :reject_third_party]),
         target when is_binary(target) <- reported_target(objects),
         false <- String.starts_with?(target, Pleroma.Web.Endpoint.url()) do
      {:reject, "[RemoteReportPolicy] Third-party: #{target}"}
    else
      _ -> {:ok, object}
    end
  end

  defp reported_target([target | _]) when is_binary(target), do: target
  defp reported_target(target) when is_binary(target), do: target
  defp reported_target(_), do: nil
