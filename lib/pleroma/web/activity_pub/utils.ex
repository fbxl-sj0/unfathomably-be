...
defmodule Pleroma.Web.ActivityPub.Utils do
  ...
  def strip_report_status_data(data) do
    ...
    new_data = put_in(data, ["object"], [actor | Enum.reverse(stripped_activities)])

    {:ok, new_data}
  end

  def get_anonymized_reporter do
    with true <- Config.get([:activitypub, :anonymize_reporter]),
         nickname when is_binary(nickname) <-
           Config.get([:activitypub, :anonymize_reporter_local_nickname]),
