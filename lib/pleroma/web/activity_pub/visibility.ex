...
defmodule Pleroma.Web.ActivityPub.Visibility do
  ...
  def visible_for_user?(%Activity{data: _, object: %Object{data: _} = object} = activity, nil) do
    restricted? =
      restrict_unauthenticated_access?(activity) or restrict_unauthenticated_access?(object)

    activity_public? = is_public?(activity) and not is_local_public?(activity)
    object_public? = is_public?(object) and not is_local_public?(object)

    not restricted? and activity_public? and object_public?
  end
