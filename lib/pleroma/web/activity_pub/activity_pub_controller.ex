...
defmodule Pleroma.Web.ActivityPub.ActivityPubController do
  ...
  def object_replies(%{assigns: assigns, query_params: params} = conn, _all_params) do
    ...
    object_ap_id = Endpoint.url() <> "/" <> object_ap_id
  ...
  def inbox(%{assigns: %{valid_signature: true, valid_host_header: false}} = conn, _params) do
    ...
    |> json("error, host header mismatch")
  end
  ...
  def internal_fetch(%Plug.Conn{host: host} = conn, _params) do
    host
    |> InternalFetchActor.configured_origin_for_host()
  ...
  # Flag activities have a distinct server-to-server path today. C2S would
  # otherwise respond as if the report had been accepted while the pipeline
  # later fails validation because there is no Flag object validator.
  defp check_allowed_action(_, %{"type" => "Flag"}) do
    {:error, "Flag activities aren't currently supported in C2S"}
  end

  # Updating a local actor profile over C2S is not supported here. Reject it
  # explicitly instead of returning 201 for an update that silently fails later.
  defp check_allowed_action(%{ap_id: ap_id}, %{"type" => "Update", "object" => %{"id" => ap_id}}) do
  ...
  defp validate_visibility(_, activity), do: {:ok, activity}
