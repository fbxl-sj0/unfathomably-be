...
defmodule Pleroma.Web.ActivityPub.Builder do
  ...
  def announce(actor, object, options \\ []) do
    actor = refresh_group_announce_actor(actor)
    visibility = announce_visibility(object, options)

    {to, cc} =
      if actor.ap_id == Relay.ap_id() do
        {[actor.follower_address], []}
      else
        announce_recipients(actor, object, visibility)
      end
  ...
  defp announce_visibility(object, options) do
    cond do
      Keyword.has_key?(options, :visibility) ->
        Keyword.fetch!(options, :visibility)

      Keyword.get(options, :public) == true ->
        "public"
  ...
  defp refresh_group_announce_actor(%User{ap_id: ap_id} = actor) when is_binary(ap_id) do
    case User.get_by_ap_id(ap_id) do
      %User{actor_type: "Group"} = fresh_actor -> fresh_actor
      _ -> actor
    end
  end

  defp refresh_group_announce_actor(actor), do: actor

  defp announce_recipients(
         %User{actor_type: "Group", follower_address: followers},
         _object,
         "public"
       ) do
    # Threadiverse community delivery:
    #
    # Lemmy, PieFed, Mbin, and Lotide-style communities announce accepted
    # posts to the public collection and to the community followers
    # collection. They do not address the original author directly.
    #
    # Addressing the author directly causes loops with stricter receivers:
    # the author's instance may fetch its own object while validating our
    # Announce and reject the packet because its local object shape is not
    # the same shape it expects to receive from another server.
    {[Pleroma.Constants.as_public()], [followers]}
  end

  defp announce_recipients(%User{follower_address: followers}, object, "public") do
    {[Pleroma.Constants.as_public(), object.data["actor"]], [followers]}
  end
  ...
  defp announce_recipients(
    ...
    {[Utils.as_local_public()], [followers]}
  end

  defp announce_recipients(%User{follower_address: followers}, object, "local") do
    recipients =
      [object.data["actor"]]
      |> Enum.filter(fn addr -> String.starts_with?(addr, Pleroma.Web.Endpoint.url() <> "/") end)

    {[Utils.as_local_public() | recipients], [followers]}
  end
  ...
  defp announce_recipients(
    ...
    {[followers], [Pleroma.Constants.as_public()]}
  end

  defp announce_recipients(%User{follower_address: followers}, object, "unlisted") do
    {[followers, object.data["actor"]], [Pleroma.Constants.as_public()]}
  end
  ...
  defp announce_recipients(%User{follower_address: followers}, object, "private") do
    {[followers, object.data["actor"]], []}
  end

  defp announce_recipients(_actor, object, "direct") do
    {[object.data["actor"]], []}
  end

  defp announce_recipients(actor, object, _visibility) do
    announce_recipients(actor, object, Visibility.get_visibility(object))
  end
