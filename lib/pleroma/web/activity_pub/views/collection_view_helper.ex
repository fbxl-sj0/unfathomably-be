...
# Copyright (c) 2017-2021 Pleroma Authors <https://pleroma.social/>
# Copyright (c) 2025 Akkoma Authors <https://akkoma.dev/>
...
defmodule Pleroma.Web.ActivityPub.CollectionViewHelper do
  ...
  def collection_page_keyset(
    ...
    |> maybe_add_activitypub_context(skip_ap_context)
  end
  ...
  defp maybe_omit_next(pagination, items, limit) when is_number(limit) do
    if Enum.count(items) < limit do
      Map.delete(pagination, "next")
    else
      pagination
    end
  end

  defp maybe_add_activitypub_context(map, true), do: map
  defp maybe_add_activitypub_context(map, false), do: Map.merge(map, Utils.make_json_ld_header())
end
