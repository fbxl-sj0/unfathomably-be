...
defmodule Pleroma.Web.StaticFE.StaticFEController do
  ...
  def show(%{assigns: %{username_or_id: username_or_id}} = conn, params) do
    with {_, %User{local: true} = user} <-
      ...
      {:fetch_user, %User{local: false} = user} ->
        RedirectController.redirector_with_meta(conn, %{user: user})

      _ ->
        if remote_profile_path?(username_or_id) do
  ...
  defp published_at(%Activity{} = activity, data) do
    data["published"] ||
      ...
      inserted_at(activity)
  end

  defp inserted_at(%Activity{inserted_at: %NaiveDateTime{} = inserted_at}) do
    inserted_at
  ...
  defp assign_id(%{path_info: ["@" <> username_or_id]} = conn, _opts),
    do: assign(conn, :username_or_id, username_or_id)
  ...
  defp remote_profile_path?(username_or_id) when is_binary(username_or_id) do
    String.contains?(username_or_id, "@")
  end
