...
defmodule Pleroma.Web.MastodonAPI.TimelineController do
  ...
  # These plugs can be collapsed into a macro once the Phoenix version in use
  # includes support for the following change:
  ...
  plug(RateLimiter, [name: :timeline, bucket_name: :public_timeline] when action == :bubble)
  ...
  def bubble(%{assigns: %{user: user}} = conn, params) do
    bubble_instances = InstanceView.local_bubble_instances()

    cond do
      Enum.empty?(bubble_instances) ->
        render_error(conn, :not_found, "bubble timeline is not configured")

      is_nil(user) and restrict_unauthenticated?(false) ->
        fail_on_bad_auth(conn)
