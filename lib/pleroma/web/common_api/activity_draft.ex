...
defmodule Pleroma.Web.CommonAPI.ActivityDraft do
  ...
  defp event_object(%__MODULE__{} = draft) do
  ...
  defp quote_post(%__MODULE__{params: %{quote_id: id}} = draft) when not_empty_string(id) do
    quote_post(%__MODULE__{draft | params: Map.put(draft.params, :quoted_status_id, id)})
  end

  defp quote_post(%__MODULE__{} = draft), do: draft
  ...
  defp quoting_visibility(%__MODULE__{} = draft), do: draft
  ...
  defp content(%__MODULE__{mentions: mentions} = draft) do
    ...
    case Utils.get_addressed_group_ap_ids(draft.params, draft.user) do
      ...
      {:error, :group_forbidden} ->
        add_error(draft, dgettext("errors", "You cannot post to this group"))
    end
  end
  ...
  defp event_location(%__MODULE__{} = draft, %Geospatial.Address{} = address) do
    location = %{
      "type" => "Place",
  ...
  defp event_location(%__MODULE__{} = draft, _), do: draft

  defp event_banner(%__MODULE__{} = draft) do
  ...
  defp with_valid(%__MODULE__{valid?: true} = draft, func), do: func.(draft)
  defp with_valid(%__MODULE__{} = draft, _func), do: draft
