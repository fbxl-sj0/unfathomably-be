    ...
    quote do
      ...
      defp set_put_layout(conn, _) do
        case Pleroma.Config.get(:app_layout, "app.html") do
          false -> put_layout(conn, html: false)
          layout -> put_layout(conn, html: {Pleroma.Web.LayoutView, layout_template(layout)})
        end
      end

      defp layout_template(layout) when is_binary(layout) do
        layout
        |> String.replace_suffix(".html", "")
        |> String.to_existing_atom()
      end

      defp layout_template(layout), do: layout
          ...
          fn plug_module, conn ->
            try do
              ...
              UndefinedFunctionError ->
                reraise "`#{plug_module}` is not skippable. Append `use Pleroma.Web, :plug` to its code.",
                        __STACKTRACE__
            end
          end
        )
