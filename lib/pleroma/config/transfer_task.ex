...
defmodule Pleroma.Config.TransferTask do
  ...
  def load_and_update_env(deleted_settings \\ [], restart_pleroma? \\ true) do
    with {_, true} <- {:configurable, Config.get(:configurable_from_database)} do
      ...
      # Prometheus and Postgrex are deliberately not restarted here. They are shared
      # infrastructure for telemetry and database access, and stopping them during
      # config transfer can destabilize the running node.
      reject = [nil, :prometheus, :postgrex]

      reject =
        if Config.get(:env) == :test do
          [:oban, :http, :not_real | reject]
        else
          reject
        end
