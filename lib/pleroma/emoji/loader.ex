...
defmodule Pleroma.Emoji.Loader do
  ...
  @type emoji_file_entry :: {String.t(), String.t(), [String.t()]}
  ...
  def load do
    ...
    test_emoji =
      if Application.get_env(:pleroma, __MODULE__, [])
         |> Keyword.get(:test_emoji, false) do
  ...
  @doc """
  Parses an `emoji.txt` stream into shortcode/file/tag tuples.

  The pack loader and pack importer both consume the same legacy comma-separated
  format, so keeping the parser here avoids small differences creeping in over
  time.
  """
  @spec load_from_file_stream(Enumerable.t(), group_patterns()) :: [emoji_file_entry()]
  def load_from_file_stream(stream, emoji_groups) do
    stream
