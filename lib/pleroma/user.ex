...
defmodule Pleroma.User do
  ...
  defdelegate get_outgoing_follow_requests(user, params \\ %{}), to: FollowingRelationship
  ...
  def binary_id(source_id) when is_binary(source_id) do
    {:ok, dumped_id} = FlakeId.Ecto.CompatType.dump(source_id)
    dumped_id
  end
  ...
  def remote_user_changeset(struct \\ %User{local: false}, params) do
    ...
    fields_limit = Config.get([:instance, :max_remote_account_fields], 0) || 0
    ...
    avatar_description = image_description_param(params, :avatar_description)
    header_description = image_description_param(params, :header_description)
  ...
  def update_changeset(struct, params \\ %{}) do
    ...
    |> put_change_if_present(
      :avatar,
      &put_upload(&1, :avatar, image_description_value(avatar_description))
    )
    |> put_change_if_present(
      :banner,
      &put_upload(&1, :banner, image_description_value(header_description))
    )
  ...
  @image_description_not_set :__image_description_not_set

  defp image_description_param(params, key) do
    case Map.fetch(params, key) do
      {:ok, value} ->
        value

      :error ->
        Map.get(params, Atom.to_string(key), @image_description_not_set)
    end
  end

  defp image_description_value(@image_description_not_set), do: nil
  defp image_description_value(value), do: value
  ...
  defp maybe_update_image_description(changeset, _image_field, @image_description_not_set) do
    changeset
  end
  ...
  def register_changeset(struct, params \\ %{}, opts \\ []) do
    ...
    |> validate_length(:bio, max: bio_limit)
    |> validate_length(:name, min: 1, max: name_limit)
    |> update_change(:registration_reason, &strip_registration_reason/1)
  ...
  defp strip_registration_reason(reason) when is_binary(reason) do
    reason
    |> HTML.strip_tags()
    |> HtmlEntities.decode()
  end

  defp strip_registration_reason(reason), do: reason
  ...
  @spec set_remote_deactivated(User.t()) :: {:ok, User.t()} | {:error, Ecto.Changeset.t()}
  ...
  def get_or_create_service_actor_by_ap_id(uri, nickname) do
    {_, user} =
      if valid_service_actor_uri?(uri) do
        case get_cached_by_ap_id(uri) do
          nil ->
            case create_service_actor(uri, nickname) do
              {:ok, user} ->
                {:ok, user}

              {:error, %{errors: errors}} ->
                case repair_service_actor_by_nickname(uri, nickname) do
                  {:ok, user} ->
                    {:ok, user}
        ...
        Logger.debug("Cannot create service actor: #{uri}/.\ninvalid service actor URI")
        {:error, nil}
      end
  ...
  defp valid_service_actor_uri?(uri) when is_binary(uri) do
  ...
  defp valid_service_actor_uri?(_), do: false

  @spec repair_service_actor_by_nickname(String.t(), String.t()) ::
          {:ok, User.t()} | {:error, any()}
  defp repair_service_actor_by_nickname(uri, nickname) do
    case get_cached_by_nickname(nickname) do
      %User{} = user ->
        user
        |> change(%{
          invisible: true,
  ...
  def follow_hashtag(%User{} = user, %Hashtag{} = hashtag) do
    ...
    with {:ok, _} <- HashtagFollow.new(user, hashtag),
         ...
         %User{} = user <- Map.put(user, :followed_hashtags, follows) do
      set_cache(user)
    end
  end
  ...
  # User delete activities use a sentinel object id. Delivery rows are never
  # written for that sentinel, so there are no delivered users to return.
