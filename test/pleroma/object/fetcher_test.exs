...
defmodule Pleroma.Object.FetcherTest do
  ...
  describe "fetching an object" do
    ...
    test "it fetches remote activity URLs without wrapping them in Create" do
      ...
      {:ok, activity} = CommonAPI.post(user, %{status: "hello from a local group"})
          ...
          %Tesla.Env{
            ...
            body:
              Jason.encode!(%{
                ...
                "type" => "Like",
                "id" => like_id,
                "audience" => "https://lemmy.example/c/general"
              })
          }
      ...
      assert {:ok, fetched_object} = Fetcher.fetch_object_from_id(like_id)
      assert fetched_object.id == object.id
      assert %Activity{data: %{"type" => "Like"}} = Activity.get_by_ap_id(like_id)

      object = Object.get_by_ap_id(object.data["id"])

      assert object.data["likes"] == [actor.ap_id]
  ...
  describe "signed fetches" do
    ...
    test_with_mock "it includes query strings and non-default ports in signed fetches",
                   Pleroma.Signature,
                   [:passthrough],
                   sign: fn _user, headers ->
                     send(self(), {:signed_fetch_headers, headers})
                     "signature"
                   end do
      clear_config([:activitypub, :sign_object_fetches], true)

      mock(fn
        %{method: :get, url: "https://mastodon.example.org:8443/objects/123?x=1"} ->
          %Tesla.Env{status: 404}
      end)

      Fetcher.fetch_and_contain_remote_object_from_id(
        "https://mastodon.example.org:8443/objects/123?x=1"
      )

      assert_receive {:signed_fetch_headers,
                      %{
                        "(request-target)": "get /objects/123?x=1",
                        host: "mastodon.example.org:8443"
                      }}
    end
