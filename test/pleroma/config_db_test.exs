...
defmodule Pleroma.ConfigDBTest do
  ...
  describe "update_or_create/1" do
    ...
    test "rejects invalid :rate_limit values" do
  ...
  describe "to_elixir_types/1" do
    ...
    defp assert_regex_equal(actual, expected) do
      assert regex_signature(actual) == regex_signature(expected)
    end

    defp regex_signature(%Regex{} = regex) do
      {Regex.source(regex), regex.opts}
    end

    test "sigil" do
      assert_regex_equal(
        ConfigDB.to_elixir_types("~r[comp[lL][aA][iI][nN]er]"),
    ...
    test "link sigil" do
      assert_regex_equal(
        ConfigDB.to_elixir_types("~r/https:\/\/example.com/"),
        ~r/https:\/\/example.com/
      )
    end
    ...
    test "link sigil with um modifiers" do
      assert_regex_equal(
        ConfigDB.to_elixir_types("~r/https:\/\/example.com/um"),
    ...
    test "link sigil with i modifier" do
      assert_regex_equal(
        ConfigDB.to_elixir_types("~r/https:\/\/example.com/i"),
    ...
    test "complex keyword with sigil" do
      ...
      assert result[:federated_timeline_removal] == []
      assert_regex_equal(List.first(result[:reject]), ~r/comp[lL][aA][iI][nN]er/)
