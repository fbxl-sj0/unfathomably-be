...
defmodule Pleroma.ListTest do
  ...
  test "getting exclusive list members" do
    user = insert(:user)
    member = insert(:user)
    other_member = insert(:user)

    {:ok, exclusive_list} = Pleroma.List.create(%{title: "exclusive", exclusive: true}, user)
    {:ok, normal_list} = Pleroma.List.create("normal", user)

    {:ok, exclusive_list} = Pleroma.List.follow(exclusive_list, member)
    {:ok, _exclusive_list} = Pleroma.List.follow(exclusive_list, other_member)
    {:ok, _normal_list} = Pleroma.List.follow(normal_list, member)

    assert Enum.sort(Pleroma.List.get_exclusive_list_members(user)) ==
             Enum.sort([member.follower_address, other_member.follower_address])
  end
