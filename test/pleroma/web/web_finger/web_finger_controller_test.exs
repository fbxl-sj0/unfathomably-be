...
defmodule Pleroma.Web.WebFinger.WebFingerControllerTest do
  ...
  test "GET host-meta" do
    ...
    assert response.resp_body =~ ~s(<?xml version="1.0" encoding="UTF-8"?>)
    assert response.resp_body =~ ~s(<XRD xmlns="http://docs.oasis-open.org/ns/xri/xrd-1.0">)
    assert response.resp_body =~ ~s(<Link )
    assert response.resp_body =~ ~s(rel="lrdd")

    assert response.resp_body =~
             ~s(template="#{Pleroma.Web.Endpoint.url()}/.well-known/webfinger?resource={uri}")

    assert response.resp_body =~ ~s(type="application/xrd+xml")
    assert response.resp_body =~ ~s(</XRD>)
  end
