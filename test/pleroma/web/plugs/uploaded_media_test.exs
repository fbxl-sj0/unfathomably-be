...
defmodule Pleroma.Web.Plugs.UploadedMediaTest do
  ...
  describe "content-type sanitization with Utils.get_safe_mime_type/2" do
    ...
    test "it defaults to generic binary data for missing configuration" do
      assert Utils.get_safe_mime_type(%{}, "image/jpeg") == "application/octet-stream"

      assert Utils.get_safe_mime_type(%{allowed_mime_types: nil}, "image/jpeg") ==
               "application/octet-stream"

      assert Utils.get_safe_mime_type(%{allowed_mime_types: ["image"]}, nil) ==
               "application/octet-stream"
    end
  end
end
