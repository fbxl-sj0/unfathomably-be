...
defmodule Pleroma.Web.Plugs.RateLimiterTest do
  ...
  test "it is disabled if remote ip plug cannot derive a forwarded IP" do
  ...
  test "doesn't crash due to a race condition when multiple requests are made at the same time and the bucket is not yet initialized" do
    ...
    refute {:error, :not_found} == RateLimiter.inspect_bucket(conn, limiter_name, opts)
  end

  @tag capture_log: true
  test "doesn't crash if rate limit scale is invalid" do
