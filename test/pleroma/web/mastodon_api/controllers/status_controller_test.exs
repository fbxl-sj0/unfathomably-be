...
defmodule Pleroma.Web.MastodonAPI.StatusControllerTest do
  ...
  describe "posting statuses" do
    ...
    test "posting a status with OGP link preview", %{conn: conn} do
      ...
      assert %{"id" => id, "card" => nil} =
               json_response_and_validate_schema(conn, 200)

      ObanHelpers.perform_all()

      assert %{"title" => "The Rock"} =
               conn
               |> recycle()
  ...
  describe "posting polls" do
    ...
    test "can have only one option", %{conn: conn} do
      conn =
        conn
        |> put_req_header("content-type", "application/json")
        |> post("/api/v1/statuses", %{
          "status" => "desu~",
          "poll" => %{"options" => ["mew"], "expires_in" => 420}
        })

      response = json_response_and_validate_schema(conn, 200)
      assert [%{"title" => "mew"}] = response["poll"]["options"]
    end

    test "cannot have zero options", %{conn: conn} do
      conn =
        conn
        |> put_req_header("content-type", "application/json")
        |> post("/api/v1/statuses", %{
          "status" => "desu~",
          "poll" => %{"options" => [], "expires_in" => 1}
        })

      %{"error" => error} = json_response_and_validate_schema(conn, 422)
      assert error == "Poll must contain at least 1 option"
    end
  end
  ...
  defp local_and_remote_activities do
    ...
    remote_note =
      insert(:note,
        user: remote_user,
        data: %{"id" => "#{remote_user.ap_id}/objects/#{Ecto.UUID.generate()}"}
      )
    ...
    remote =
      insert(:note_activity,
        ...
        user: remote_user,
        note: remote_note,
        data_attrs: %{"id" => "#{remote_user.ap_id}/activities/#{Ecto.UUID.generate()}"}
      )
  ...
  test "context hydrates remote comments collections before listing descendants" do
    ...
    parent_object_id = "https://remote.example/objects/parent"
    parent_activity_id = "https://remote.example/activities/parent"
    comments_id = parent_object_id <> "/comments"
    reply_object_id = "https://remote.example/objects/reply"
    ...
    parent_activity =
      insert(:note_activity,
        ...
        user: remote_user,
        note: parent_note,
        data_attrs: %{"id" => parent_activity_id, "context" => context}
      )

    Tesla.Mock.mock(fn
      %{method: :get, url: ^parent_object_id} ->
        %Tesla.Env{
          status: 200,
        ...
        %Tesla.Env{
          ...
          body:
            Jason.encode!(%{
              "id" => comments_id,
              "type" => "Collection",
              "items" => [reply_object_id]
            })
        }

      %{method: :get, url: ^reply_object_id} ->
        %Tesla.Env{
          status: 200,
          headers: HttpRequestMock.activitypub_object_headers(),
          body:
    ...
    assert [%{"content" => content}] = response["descendants"]
    assert content =~ "Remote comment"
  end
  ...
  test "favorites paginate correctly" do
    ...
    [_, next_url] = Regex.run(~r/^<([^>]+)>/, next)

    max_id =
      next_url |> URI.parse() |> Map.get(:query) |> URI.decode_query() |> Map.fetch!("max_id")
  ...
  test "expires_at is nil for another user" do
    ...
    assert abs(DateTime.diff(expires_at, a_expires_at)) <= 1
  ...
  describe "translating statuses" do
    ...
    test "it returns an error if target language is unsupported" do
      clear_config([Pleroma.Language.Translation, :provider], UnsupportedLanguageTranslationMock)
