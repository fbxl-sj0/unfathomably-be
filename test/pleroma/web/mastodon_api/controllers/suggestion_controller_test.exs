...
defmodule Pleroma.Web.MastodonAPI.SuggestionControllerTest do
  ...
  test "tolerates malformed v2 suggestion limits", %{conn: conn} do
    %{id: user_id} = insert(:user, is_suggested: true)

    res =
      conn
      |> get("/api/v2/suggestions?limit=not-a-number")
      |> json_response_and_validate_schema(200)
  ...
  test "dismiss missing suggestion returns not found", %{conn: conn} do
    assert %{"error" => "Record not found"} =
             conn
             |> delete("/api/v1/suggestions/404404")
