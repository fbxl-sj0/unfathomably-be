...
defmodule Pleroma.Web.MastodonAPI.AccountViewTest do
  ...
  setup do
    mock(fn env -> HttpRequestMock.request(env) end)
    :ok
  end
  ...
  describe "hiding follows/following" do
    ...
    test "does not count dormant remote follow edges for local accounts" do
  ...
  test "uses mediaproxy urls when it's enabled (regardless of media preview proxy state)" do
    ...
    with media_preview_enabled <- [false, true] do
      ...
      rendered = AccountView.render("show.json", %{user: user, skip_visibility_check: true})
