...
defmodule Pleroma.Web.MastodonAPI.NotificationViewTest do
  ...
  defp test_notifications_rendering(notifications, user, expected_result) do
    ...
    assert_group_keys(notifications, result)
    assert expected_result == without_group_keys(result)
  ...
  defp assert_group_keys(notifications, result) do
    expected_group_keys = Enum.map(notifications, &Notification.group_key/1)
    actual_group_keys = Enum.map(result, &Map.fetch!(&1, :group_key))

    assert expected_group_keys == actual_group_keys
  end

  defp without_group_keys(result) do
    Enum.map(result, &Map.delete(&1, :group_key))
  end
