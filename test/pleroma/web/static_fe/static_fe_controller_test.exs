...
defmodule Pleroma.Web.StaticFE.StaticFEControllerTest do
  ...
  describe "user profile html" do
    ...
    test "profile shorthand route as HTML", %{conn: conn, user: user} do
      conn = get(conn, "/@#{user.nickname}")
  ...
  describe "notice html" do
    ...
    test "single notice page with missing object published date", %{conn: conn, user: user} do
      {:ok, activity} = CommonAPI.post(user, %{status: "testing a dateless thing!"})
