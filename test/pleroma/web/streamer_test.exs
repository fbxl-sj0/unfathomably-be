...
defmodule Pleroma.Web.StreamerTest do
  ...
  describe "federated target stream topics" do
    test "resolves group and source stream topics" do
      group =
        insert(:user,
          local: false,
          actor_type: "Group",
      ...
      group_topic = "group:#{group.id}"
      source_topic = "source:#{source.id}"

      assert {:ok, ^group_topic} =
               Streamer.get_topic("group", nil, nil, %{"group" => group.ap_id})

      assert {:ok, ^group_topic} =
               Streamer.get_topic("group:#{group.ap_id}", nil, nil, %{})
    ...
    test "publishes public group and source creates to target streams" do
      ...
      topics = Pleroma.Activity.Ir.Topics.get_activity_topics(activity)
    ...
    test "rejects blank and oversized group and source stream identifiers" do
      ...
      assert {:error, :bad_topic} = Streamer.get_topic("group", nil, nil, %{"group" => ""})
      assert {:error, :bad_topic} = Streamer.get_topic("group", nil, nil, %{"group" => "   "})

      assert {:error, :bad_topic} =
               Streamer.get_topic("group", nil, nil, %{"group" => oversized_identifier})

      assert {:error, :bad_topic} =
               Streamer.get_topic("group:#{oversized_identifier}", nil, nil, %{})

      assert {:error, :bad_topic} = Streamer.get_topic("source", nil, nil, %{"source" => ""})

      assert {:error, :bad_topic} =
               Streamer.get_topic("source", nil, nil, %{"source" => oversized_identifier})

      assert {:error, :bad_topic} =
               Streamer.get_topic("source:#{oversized_identifier}", nil, nil, %{})
    end

    test "does not publish non-public group and source creates to target streams" do
      group =
        insert(:user,
          local: false,
          actor_type: "Group",
          ap_id: "https://groups.example/c/private",
          nickname: "private@groups.example"
        )
      ...
      source =
        insert(:user,
          local: false,
          actor_type: "Service",
          ap_id: "https://funkwhale.example/channels/private-library",
          nickname: "private-library@funkwhale.example"
        )
