...
defmodule Pleroma.Web.ActivityPub.ActivityPubControllerTest do
  ...
  describe "/internal/fetch" do
    ...
    test "it uses an HTTPS fetch actor origin matching the request host", %{conn: conn} do
      clear_config([:activitypub, :fetch_actor_origin], "https://fetch.example/internal-base")

      res =
        %{conn | host: "fetch.example"}
        |> get(activity_pub_path(conn, :internal_fetch))
        |> json_response(200)

      assert res["id"] == "https://fetch.example/internal-base/internal/fetch"
    end

    test "it ignores an HTTP fetch actor origin unless explicitly allowed", %{conn: conn} do
      clear_config([:activitypub, :fetch_actor_origin], "http://fetch.example:4000")
      clear_config([:activitypub, :allow_http_fetch_actor_origin], false)
      ...
      refute String.starts_with?(res["id"], "http://fetch.example:4000/")
    end

    test "it allows HTTP fetch actor origins for explicit smoke configurations", %{conn: conn} do
      clear_config([:activitypub, :fetch_actor_origin], "http://fetch.example:4000")
      clear_config([:activitypub, :allow_http_fetch_actor_origin], true)
      ...
      assert res["id"] == "http://fetch.example:4000/internal/fetch"
    end

    test "it ignores HTTP fetch actor origins in production", %{conn: conn} do
      clear_config(:env, :prod)
      clear_config([:activitypub, :fetch_actor_origin], "http://fetch.example:4000")
      clear_config([:activitypub, :allow_http_fetch_actor_origin], true)
