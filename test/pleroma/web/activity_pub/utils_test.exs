...
defmodule Pleroma.Web.ActivityPub.UtilsTest do
  ...
  describe "strip_report_status_data/1" do
    test "reduces reported statuses to ActivityPub object ids" do
      reporter = insert(:user)
      target_account = insert(:user)
      {:ok, activity} = CommonAPI.post(target_account, %{status: "foobar"})

      note_id = activity.object.data["id"]
      ...
      assert {:ok, stripped} = Utils.strip_report_status_data(report)
      assert stripped.data["object"] == [target_account.ap_id, note_id, note_id, note_id]
    end
  end
