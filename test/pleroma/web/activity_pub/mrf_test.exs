...
defmodule Pleroma.Web.ActivityPub.MRFTest do
  ...
  test "subdomains_regex/1" do
    assert_regexes_equal(MRF.subdomains_regex(["unsafe.tld", "*.unsafe.tld"]), [
      ~r/^unsafe.tld$/i,
      ~r/^(.*\.)*unsafe.tld$/i
    ])
  end
  ...
  describe "subdomain_match/2" do
    test "common domains" do
      ...
      assert_regexes_equal(regexes, [~r/^unsafe.tld$/i, ~r/^unsafe2.tld$/i])
    ...
    test "wildcard domains with one subdomain" do
      ...
      assert_regexes_equal(regexes, [~r/^(.*\.)*unsafe.tld$/i])
  ...
  defp assert_regexes_equal(actual, expected) do
    assert Enum.map(actual, &regex_signature/1) == Enum.map(expected, &regex_signature/1)
  end
