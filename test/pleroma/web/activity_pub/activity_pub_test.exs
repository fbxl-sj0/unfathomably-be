...
defmodule Pleroma.Web.ActivityPub.ActivityPubTest do
  ...
  describe "building a user from his ap id" do
    ...
    test "works with actor image urls as arrays" do
      user_id = "https://example.com/users/image-array"

      user_data =
        "test/fixtures/users_mock/user.json"
        |> File.read!()
        |> String.replace("{{nickname}}", "image-array")
        |> Jason.decode!()
        |> Map.delete("featured")
        |> Map.put("icon", %{"type" => "Image", "url" => ["https://example.com/avatar.webp"]})
        |> Map.put("image", %{"type" => "Image", "url" => ["https://example.com/banner.gif"]})
        |> Jason.encode!()
      ...
      assert user.avatar["url"] == [%{"href" => "https://example.com/avatar.webp"}]
      assert user.banner["url"] == [%{"href" => "https://example.com/banner.gif"}]
    end
    ...
    test "works with alsoKnownAs as a string" do
      user_id = "https://example.com/users/hubzilla"
      also_known_as = "https://example.com/channel/old-hubzilla-identity"

      user_data =
        "test/fixtures/users_mock/user.json"
        |> File.read!()
        |> String.replace("{{nickname}}", "hubzilla")
        |> Jason.decode!()
        |> Map.delete("featured")
        |> Map.put("alsoKnownAs", also_known_as)
        |> Jason.encode!()
    ...
    test "stores Misskey source profile text separately from the HTML summary" do
      user_id = "https://example.com/users/misskey-source"
      raw_summary = "I write MFM here :blobcat:"
      html_summary = "<p>I write MFM here <img alt=\":blobcat:\"></p>"

      user_data =
        "test/fixtures/users_mock/user.json"
        |> File.read!()
        |> String.replace("{{nickname}}", "misskey-source")
        |> Jason.decode!()
        |> Map.delete("featured")
        |> Map.put("summary", html_summary)
        |> Map.put("_misskey_summary", raw_summary)
        |> Jason.encode!()
  ...
  describe "reply filtering" do
    ...
    # we currently use for detecting to who a reply is directed. Keep it
    # as skipped coverage for the next reply visibility rewrite.
    @tag skip: true
