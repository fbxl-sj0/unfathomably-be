...
defmodule Pleroma.Web.PleromaAPI.ChatControllerTest do
  ...
  defp assert_pagination_link(link, chat_id, rel, cursor_params) do
    assert [_, url, actual_rel] = Regex.run(~r/^<(.+)>; rel="([^"]+)"$/, link)
    assert actual_rel == rel

    uri = URI.parse(url)
    assert uri.path == "/api/v1/pleroma/chats/#{chat_id}/messages"

    params = URI.decode_query(uri.query || "")
    assert params["limit"] =~ ~r/^\d+$/

    Enum.each(cursor_params, fn param ->
