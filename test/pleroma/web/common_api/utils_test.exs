...
defmodule Pleroma.Web.CommonAPI.UtilsTest do
  ...
  describe "format_input/3" do
    ...
    test "sanitizes text/html after linkification" do
      text = ~s|<p>safe</p><a href="javascript:alert(1)">bad</a><script>alert(1)</script>|

      {output, [], []} = Utils.format_input(text, "text/html")

      assert output =~ "<p>safe</p>"
      refute(output =~ "javascript:")
      refute output =~ "<script"
    end

    test "preserves harmless MFM presentation hints" do
      text =
        ~s|<span class="mfm-fade" data-mfm-out="true" data-mfm-loop="true" data-mfm-rotateByVelocity="true"><script>alert(1)</script>soft landing</span>|

      {output, [], []} = Utils.format_input(text, "text/html")

      assert output =~ ~s(class="mfm-fade")
      assert output =~ ~s(data-mfm-out="true")
      assert output =~ ~s(data-mfm-loop="true")
      assert output =~ ~s(data-mfm-rotatebyvelocity="true")
      assert output =~ "soft landing"
      refute output =~ "<script"
    end
