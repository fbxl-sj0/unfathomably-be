...
RUN apt-get update &&\
    apt-get install -y git elixir erlang-dev erlang-nox build-essential cmake libssl-dev libmagic-dev libvips-dev automake autoconf libncurses5-dev &&\
    mix local.hex --force &&\
...
RUN apt-get update &&\
    apt-get install -y --no-install-recommends curl ca-certificates imagemagick libmagic-dev libvips42 ffmpeg libimage-exiftool-perl libncurses5 postgresql-client fasttext &&\
